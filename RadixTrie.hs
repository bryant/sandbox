...
node_upsert [] v (Branch _ rest) = Branch (Just v) rest
node_upsert [] vnew (Leaf vold []) = Leaf vnew []
node_upsert [] vnew (Leaf vold (r:rs)) =  -- evict old to one level down
    Branch (Just vnew) (Map.singleton r $ Leaf vold rs)
node_upsert (k:ks) v (Branch u next) = case Map.lookup k next of
    Nothing -> Branch u $ Map.insert k (Leaf v ks) next
    Just further -> Branch u $ Map.insert k (node_upsert ks v further) next
node_upsert kks@(_:_) vnew (Leaf vold rrs) = case lcp kks rrs of
...
upsert trie (k:ks) v = case Map.lookup k trie of
    ...
    Just node -> Map.insert k (node_upsert ks v node) trie
